...
KellyTools.xmlRequest = function(urlOrig, cfg, callback) {
    ...
    var xhr = new XMLHttpRequest();
        ...
        if (cfg.formData) xhr.send(cfg.formData) : xhr.send();  
        
        controller.abortController = xhr;
        return controller;
}            

// todo - add responseType support, currently used only for download media data (return always blob)

KellyTools.fetchRequest = function(urlOrig, cfg, callback) {
    
    var fetchRequest = new Object();
        fetchRequest.canceled = false;
        fetchRequest.abortController = new AbortController();
    ...
    .then(function(response) {
        ...
        if (response.status == 200) {
            ...
            return response.blob().then(function(blob) {
                ...
                callback(urlOrig, blob, false, false, fetchRequest);
            });

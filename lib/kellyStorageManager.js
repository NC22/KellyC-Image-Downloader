...
function KellyFavStorageManager(cfg) {
    ...
    this.applayDBCreateButton = function(el, idPrefix) {
        ...
        el.onclick = function() {
            ...
            mode = 'cancel';
            if (document.getElementById(idPrefix + '-add').checked) mode = 'add';
            if (document.getElementById(idPrefix + '-overwrite').checked) mode = 'overwrite';
                ...
                var createProfileByInput = function(input, fileData) {
                    ...
                    if (newDBData) { 
                        ...
                        if (!KellyTools.DEBUG && typeof newDBData.profile != 'undefined' && env.profile && newDBData.profile != env.profile) {
                            handler.showMessage(lng.s('', 'storage_create_e3', { PROFILE_MODULE : newDBData.profile, CURRENT_MODULE : env.profile}), true);
                            return;
                        }
    ...
    this.showUnsafeStoreWarning = function(context) {
        ...
        if (context == 'db_manager') {
           ...
           if (show && !handler.fav.getGlobal('fav').coptions.ignoreStoreWarning) {
                ...
                var env = handler.fav.getGlobal('env'), tooltip = KellyTools.getNoticeTooltip(env.hostClass, env.className), html = '';
        ...
        var html = '\
            <div class="' + handler.className + '-wrap">\
                <table class="' + handler.className + '-options-table">\
                    ...
                    <tr><td><label for="' + idPrefix + '-overwrite">' + lng.s('Перезаписать если существует', 'storage_create_overwrite_if_exist') + '</label></td>\
    ...
    */
    ...
    this.validateCfg = function(data) {
        ...
        if (!data.coptions.unlock) {
            data.coptions.unlock = {censored : true, censoredAuto : false, censoredParser : false};
        }
        ...
        if (typeof data.coptions.unlockCensored == 'undefined') data.coptions.unlockCensored = true;
        ...
        // data.coptions.mobileOptimization = false; // todo
        // data.coptions.darkTheme = false;

...
function KellyFavStorageManager(cfg) {
    ...
    this.showUnsafeStoreWarning = function(context) {
        
        var show = handler.driver == 'localstorage' && KellyTools.getBrowserName() == 'firefox';
        
        if (context == 'db_manager') {
            var cell = KellyTools.getElementByClass(handler.storageContainer, handler.className + '-options-fox-warning');
            if (cell) cell.style.display = show ? 'table-cell' : 'none';
        } else {
           
           if (show && !handler.fav.getGlobal('fav').coptions.ignoreStoreWarning) {
               
                var env = handler.fav.getGlobal('env'), tooltip = new KellyTooltip({
                    target : 'screen', offset : {left : 40, top : -40}, 
                ...
                var html = '';
                for (var i = 2; i <= 5; i++) html += lng.s('', 'storage_type_notice_fox_warning_' + i);
                
                KellyTools.setHTMLData(tooltip.getContent(), html);

                var action = function() { 
                
                    if (this.getAttribute('data-action') == 'ok') {
                        
                        handler.fav.getGlobal('fav').coptions.ignoreStoreWarning = true;            
                        handler.fav.save('cfg');  
                    } else {        
                        handler.fav.showOptionsDialog(env.className + '-Storage');
                        window.scrollTo(0,  handler.wrap.getBoundingClientRect().top + KellyTools.getScrollTop());
                ...
                var buttons = tooltip.getContent().getElementsByTagName('A');
                for (var i = 0; i < buttons.length; i++)  buttons[i].onclick = action;
    ...
    this.showDBManager = function(idPrefix) {
        ...
        handler.showUnsafeStoreWarning('db_manager');
        ...
        var html = '\
            <div class="' + handler.className + '-wrap">\
                <table class="' + handler.className + '-options-table">\
                    ...
                    <tr><td colspan="2" class="' + handler.className + '-options-fox-warning" style="background: #00000017; padding : 6px; display : none;">' + lng.s('', 'storage_type_notice_fox_warning') + '</td></tr>\
    ...
    */
    ...
    this.categoryCreate = function(db, name, catIsNSFW, order) {
        ...
        if (!name || handler.searchCategoryBy(db, name, 'name') !== false) return false;
            
        if (!order) order = db.categories.length + 1;
          
        db.ids++;
        db.categories.push({ 
            name : name, 
            id : db.ids, 
            nsfw : catIsNSFW ? true : false,
            order : order, 
    ...
    function setDBCache(name, option, value) {
        ...
        return; // todo - add cache for db items list \ get size | partly tested, currently unused
    ...
    /* 
        ...
        input :
        { itemIndex : false || int, images : [], cats : [], info : {text, name, dimensions : {width , height}}, post || postLink, comment || commentLink }
         
    */
    
    this.createDbItem = function(data, db, update) {
        ...
        if (itemIndex === false) itemIndex = handler.searchItem(db, postItem);

            ...
            if (request.images) {
                ...
                rImages : for (var i = 0; i < request.images.length; i++) {
                    ...
                    for (var b = 0; b < request.images[i].relatedSrc.length; b++) {   
                        currentCache.push(request.images[i].relatedSrc[b].split('#KELLY_GROUP#')[0]);
                        if (KellyEDRecorder.recorder.cache.indexOf(currentCache[currentCache.length-1]) != -1) continue rImages;                        
                    }
                    
                    KellyEDRecorder.recorder.cache = KellyEDRecorder.recorder.cache.concat(currentCache);

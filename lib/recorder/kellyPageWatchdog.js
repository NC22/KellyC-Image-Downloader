function KellyPageWatchdog(cfg) 
{
    ...
    this.parseImagesDocByDriver = function(thread) {
        ...
        if (handler.url.indexOf('pixiv') != -1 ) {
           ...
           thread.response = ''; 
            ...
            if (image) handler.imagesPool.push({relatedSrc : [KellyPageWatchdog.addSrcGroup(image, thread.rules.indexOf('mark_comment=1') != -1 ? 'vkComment' : false)]});
            ...
            if (image) handler.imagesPool.push({relatedSrc : [KellyPageWatchdog.addSrcGroup(image.getAttribute('src'), thread.rules.indexOf('mark_comment=1') != -1 ? 'vkComment' : false)]});
    ...
    this.addItemByDriver = function(el, item) {
        ...
        if (handler.url.indexOf('pinterest') != -1 && el.getAttribute('src') && el.getAttribute('srcset')) {
            
            // related doc keeped upper if needed <a href=[]>...<div>...<img>...<////>
            validateSrc(item, el.src, 'addSrcFromAttributes-src', el);
            
            var srcs = el.getAttribute('srcset').split(',');             
            for (var i = 0; i < srcs.length; i++) validateSrc(item, srcs[i].trim().split(' ')[0], 'addSrcFromAttributes-src', el);

            return item.relatedSrc.length > 0 ? addDriverAction.ADD : addDriverAction.SKIP;
            
        } else if (handler.url.indexOf('vk.com') != -1 && (el.classList.contains('page_doc_photo_href') || el.classList.contains('page_post_thumb_unsized'))) { 
               
            item.relatedDoc = el.href;      
            if (KellyTools.getParentByClass(el, 'reply_content')) item.relatedDoc += '##FETCH_RULES##mark_comment=1';
            if (el.getAttribute('data-thumb')) validateSrc(item, el.getAttribute('data-thumb'), 'addSrcFromAttributes-src', el); else addSrcFromStyle(el, item);
            if (item.relatedSrc.length > 0) {
                item.relatedSrc[0] = KellyPageWatchdog.addSrcGroup(item.relatedSrc[0], 'imagePreview');
                return addDriverAction.ADD;
            } else return addDriverAction.SKIP;
            ...
            if (el.getAttribute('onclick').indexOf('showPhoto') != -1) {
    ...
    this.validateItemByDriver = function(item) { // thumb.php - hentai foundry
        ...
        var matches = [
            ...
            {url : 'vk.com', patternsDoc : [['al_photos.php', 'imagePreview']]},
        ...
        for (var i = 0; i < matches.length; i++) {
            if (handler.url.indexOf(matches[i].url) != -1) {  
                ...
                item.relatedSrc.forEach(function(src, index) {
                    if (group.length > 0) item.relatedSrc[index] = KellyPageWatchdog.addSrcGroup(src, group); // group by relatedDoc
                    else { // check source by source
                        group = matchPatterns(src, matches[i].patterns);
                        if (group.length > 0) item.relatedSrc[index] = KellyPageWatchdog.addSrcGroup(src, group);
    ...
    */
    ...
    var validateSrc = function(item, src, context, el, groups) {
        ...
        if (groups) src = KellyPageWatchdog.addSrcGroup(src, groups);
    ...
    var addSrcFromAttributes = function(el, item) {
       ...
       for (var i = el.attributes.length - 1; i >= 0; i--) {
            ...
            var link = el.attributes[i].value; 
            if (item.relatedSrc.indexOf(link) == -1) validateSrc(item, link, 'addSrcFromAttributes-' + el.attributes[i].name, el);
       }              
    }
            
    var addSrcFromStyle = function(el, item) {
        if (el.getAttribute('style') && el.getAttribute('style').indexOf('url(') != -1) {
            ...
            validateSrc(item, link, 'addSrcFromStyle', el, 'imageBg');
        }
    }
    ...
    var parseItem = function(el) {
        ...
        var driverResult = handler.addItemByDriver(el, item);
             ...
             if (driverResult == addDriverAction.ADD) return itemAdd(item);
        ...
        if (el.tagName == 'SOURCE' && el.parentElement && el.parentElement.tagName == 'VIDEO') {
            if (validateSrc(item, el.getAttribute('src'), 'srcVideo', el, 'srcVideo')) return itemAdd(item);
        ...
        if (el.tagName == 'LINK' && el.getAttribute('rel') && el.getAttribute('rel').indexOf('icon') != -1) {
            ...
            if (validateSrc(item, el.href ? el.href : el.src, 'srcIcon', el, 'srcIcon')) return itemAdd(item);
    ...
    function getApiMessage(request, sender, callback) {
        ...
        if (request.method == "parseImages") {       
            ...
            handler.imagesPool = []; handler.srcs = [];
            handler.parseImages(); 
...
KellyPageWatchdog.gS = '#KELLY_GROUP#'; // todo #KELLY_GROUP#

KellyPageWatchdog.explodeSrc = function(src) {
     var groups = [];
     var src = src.split(KellyPageWatchdog.gS);
         src.forEach(function(group, index) { if (index > 0) groups.push(group); });
         src = src[0].trim();
         
    return {src : src, groups : groups};
}

KellyPageWatchdog.addSrcGroup = function(src, group) {
    if (!group) return src;
    
    var groups = typeof group == 'string' ? [group] : group;
    
    groups.forEach(function(group) {
        group = group.trim();
        if (group) src += KellyPageWatchdog.gS + group;
    });        

    ...
    */
            ...
            onEnd : function(reason) {
                 ...
                 for (var i = 0; i < handler.docsImages.length; i++) {
                    ...
                    if (!handler.docsImages[i].relatedItem.pw || (handler.docsImages[i].relatedItem.pw <= handler.docsImages[i].pw && handler.docsImages[i].relatedItem.ph <= handler.docsImages[i].ph)) {
                        ...
                        // todo - optional skip other if one original found ?
    ...
    handler.onDownloadDoc = function(self, thread, jobsLength) {
        ...
        if (!thread.response) {
            ...
            handler.parser.untrustedData = false; // Trust by default if some thing added by external site driver
...
KellyLoadDocControll.initOptions = function(optionsManager, options) {
    ...
    if (!options.recorderThread) options.recorderThread = {};
    var notice = false;
    ...
    for (var k in KellyLoadDocControll.threadDefaults){
        ...
        if (typeof options.recorderThread[k] == 'undefined') options.recorderThread[k] = KellyLoadDocControll.threadDefaults[k];

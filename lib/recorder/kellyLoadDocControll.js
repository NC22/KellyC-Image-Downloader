...
{    
    ...
    function constructor(cfg) {
        ...
        handler.thread = new KellyThreadWork({
            pauseEvery : [300],
            pauseTimer : [1],
            timeout : 5,
            timeoutOnEnd : [0.1],
            maxThreads : 1,
            validateResponseMethod : false,
        }); 
    ...
    handler.onDownloadDoc = function(self, thread, jobsLength) {
        ...
        if (!thread.response) {
            ...
            for (var i = 0; i < handler.parser.imagesPool.length; i++) {
                handler.parser.imagesPool[i].relatedSrc.forEach(function(src) {
                    
                    var unpackedSrc = KellyPageWatchdog.explodeSrc(src);
                        unpackedSrc.relatedItem = thread.job.data;
                        unpackedSrc.untrustedData = handler.parser.untrustedData;
                        
                    handler.docsImages.push(unpackedSrc);
                });
            }
        }
    ...
    handler.updateState = function(context) {
        ...
        if (stage == 'loadDoc') {
            ...
            stat = { total : total, current : total - handler.thread.getJobs().length };
            
        } else if ( stage == 'loadImg') {
            
            stat = { total : total, current : 0 };
            
            for (var i = 0; i < handler.docsImages.length; i++) {
                if (!handler.docsImages[i].untrustedData || handler.docsImages[i].pw) stat.current++;
            }
        }

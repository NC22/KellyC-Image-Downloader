    ...
    */
    ...
    handler.onDownloadDoc = function(self, thread, jobsLength) {
        ...
        if (!thread.response) {
            ...
            handler.parser.setLocation({url : thread.job.url, host : KellyTools.getLocationFromUrl(thread.job.url).origin}); // KellyTools.getLocationFromUrl(thread.job.url).hostname

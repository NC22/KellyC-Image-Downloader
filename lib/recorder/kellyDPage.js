...
KellyDPage.tileMapControll = {
    srcs : [], maps : [],
    addUrlTileMapData : function(url, referrer) {
    
        url = url.split(';')[1].split(',');            
        var data = {tileMapSrc : url[0], x : url[1], y : url[2], width : url[3], height : url[4]};
        
        // create black image before load tilemap image
        ...
        var canvas = document.createElement('canvas');
         ...
         data.mapKey = KellyDPage.tileMapControll.srcs.indexOf(data.tileMapSrc);         
         if (data.mapKey == -1) {
            
            KellyDPage.addUrlMapItem(KellyTools.getUrlExt(data.tileMapSrc), KellyTools.getLocationFromUrl(data.tileMapSrc), referrer);
            KellyDPage.tileMapControll.srcs.push(data.tileMapSrc);
            data.mapKey = KellyDPage.tileMapControll.srcs.length-1;
            KellyDPage.tileMapControll.maps[data.mapKey] = {src : data.tileMapSrc, tiles : [], image : false};
         }
    ...
    initTileMapLoad : function(tm) {
        ...
        tm.image.onload = function() {
            ...
            for (var i = 0; i < tm.tiles.length; i++) {
                ...
                canvas.getContext('2d').drawImage(tm.image, item.x, item.y, item.width, item.height, 0, 0, item.width, item.height);
                item.postItem.pImage = canvas.toDataURL();                
            }    
...
KellyDPage.addStorageItem = function(src, doc, referrer, groups) {
     ...
     if (ext == 'dataUrl' && src.indexOf('tilemap') != -1) {
         tileMapData = KellyDPage.tileMapControll.addUrlTileMapData(src, referrer);
     ...
     if (tileMapData) {
         tileMapData.postItem = itemData.postItem;
         KellyDPage.tileMapControll.maps[tileMapData.mapKey].tiles.push(tileMapData);
     }
...
KellyDPage.showAdditionFilters = function() {
    ...
    var cl = KellyDPage.env.className, section = document.getElementById(cl + '-sidebar-wrap'), html = '', collapsedExtra = 'collapsed'; // (K_FAV.mobileOptimization ? 'collapsed' : '');
    ...
    html += '<div class="' + cl + '-ModalBox ' + cl + '-Main-download-btn-ModalBox"><a href="#" class="' + cl + '-Main-download-btn" >Скачать</a></div>';
...
KellyDPage.init = function() {
     ...
     KellyDPage.env.events.onDisplayBlock = function(mode, action, oldMode) {
         ...
         var downloadButton = KellyTools.getElementByClass(KellyDPage.commonFilters, KellyDPage.env.className + '-Main-download-btn');
         if (downloadButton) {

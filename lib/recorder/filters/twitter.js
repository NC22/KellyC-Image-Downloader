...
KellyRecorderFilterTwitter.addItemByDriver = function(handler, el, item) {
    if (handler.url.indexOf('twitter') != -1 && el.tagName == 'IMG' && el.src.indexOf('name=') != -1 && el.src.indexOf('pbs.twimg.com/media') != -1) {
       ...
       handler.addSingleSrc(item, el.src, 'handler.addSrcFromAttributes-src', el, 'imagePreview');
       handler.addSingleSrc(item, el.src.split('&name=')[0], 'handler.addSrcFromAttributes-src', el, 'imageOriginal');

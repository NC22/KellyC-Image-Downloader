...
Метод __addItemByDriver__ вызывается перед обработкой DOM элемента el (проверяется каждый Node.ELEMENT_NODE элемент документа) страницы методом поумолчанию (kellyPageWatchDog.parseItem). 

Метод (_kellyPageWatchDog.parseItem_) если фильтр не завершает \ заменяет его выполнение, в общем случае собирает ссылки (из непопадающих в исключение заданых элементу атрибутов методом getAttribute) определенные как _возможные картинки_ из элемента el в объект <b>item</b> и пробует найти ассоциируемую с картинкой ссылку на документ. (выполняется поиск структур вида : ` <a href="документ содержащий оригинал">...<img src="превью">...</a> `) и добавляет итоговый объект в общий список.

структура объекта <b>item</b>
...
object 
{
  ...
  relatedGroups : [array of [srcIndex => [array of group names \ keys] ...]],
  referrer : string - url сгенерированный на основе хоста таба с которого была запрошена картинка
}
```

[group names \ keys] - название или идентификатор группы может быть любым. Заранее созданные группы и их возможные настройки.
Дополнить массив групп можно добавив соответствующие пары ключ - настройки в массив handler.additionCats (доп. группы можно задать заранее или в процессе обработки элементов)
...
атрибуты исключенные из обработки по умолчанию : 'name', 'class', 'id', 'type', 'alt', 'title', 'data-md5'. Фоновая картинка из атрибута style, учитывается.
...
_предполагаемая ссылка на изображение_ (массив item.relatedSrc) определяется исходя из нескольких условий - уровня доверия конкретному тегу (IMG | SOURCE | DIV ... ) в зависимости от его названия \ ключа атрибута тега \ возможности предположить расширение файла исходя из строки ссылки. Это позволяет отсеивать строки которые точно не могут являтся ссылками (общий метод валидации см. kellyPageWatchDog.addSingleSrc)

Заранее явно исключить отдельные ссылки по совпадению строки можно добавив строку в массив KellyPageWatchdog.bannedUrls
...
Можно переопределять конфигурацию установленную в настройках пользователя по умолчанию для потокового загрузчика _docLoader_ через метод __docLoader.parser.updateCfg__

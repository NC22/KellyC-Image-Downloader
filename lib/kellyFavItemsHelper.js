...
function KellyFavItemsHelper() 
{
    ...
    function getInitAction() { // if page included as Iframe, we use it just to restore local storage data on subdomain, or domain with other name
        var mode = findGetParameter(env.actionVar, true);
        if (!mode) return 'main';
    ...
    function findGetParameter(parameterName, get) {
	
		if (!get) {
		
			var lstring = window.location.href.split('#');
			
			if (!lstring || lstring.length < 2) return null;
			else lstring = lstring[1];
			
		} else {
		
			var lstring = location.search;
		}
    ...
    this.load = function(type, onAfterload) {
		...
		var onLoadItems = function(itemsDb) {
			...
			if (!itemsDb) {
				...
				log(fav.coptions.storage + ' db not exist, default used');
			}
		...
		var onLoadConfig = function (config) {
			...
			else log('bad or empty config ' + sm.prefixCfg + 'config, default used');
            ...
            if (!type || type == 'items') {
				...
				handler.getStorageManager().saveDB(fav.coptions.storage, { 
					...
					ids : fav.ids, 
					  ...
					  log('save data to storage ' + (error ? error : 'OK'));
				});
            }
			...
			if (!type || type == 'cfg') {
				...
				handler.getStorageManager().saveDB('config', { 
					...
					coptions : fav.coptions
					 ...
					 log('save cfg to storage ' + (error ? error : 'OK'));
				}, true);
			}
        }
    }
    ...
    this.loadDirectly = function() {
        ...
        if (document.getElementById(env.mainContainer)) return;
        ...
        var mainContainer = document.createElement('div');
            mainContainer.id = env.mainContainer;
	...
	function addTipToEl(el, message, cfg, delay) {
	...
	function addImageInfoTip(el) {
		...
		var getMessage = function(el, e) {
			...
			var text = 'Запись №' + item.id + '<br>';
			if (item.pw ) {
				text += item.pw + 'x' + item.ph;
				if (item.ps ) text += ' (оригинал)' + '<br>';
				else text += ' (превью)' + '<br>';
			}
			
			if (item.link) {
				text += '<a href="' + item.link + '" target="_blank">Перейти к публикации</a>' + '<br>';
				
			}
			
			if (item.commentLink) {
				text += '<a href="' + item.commentLink + '" target="_blank">Перейти к комментарию</a>' + '<br>';
				
			}
			
			if (typeof item.pImage != 'string' && item.pImage.length > 1) {
				text += 'Изображений : ' + item.pImage.length + '<br>';
			}
			...
			if (typeof item.pImage != 'string' && item.pImage.length > 1) {
				...
				text += '<ul class="'+ env.className +'-ItemTip-images">';
					...
					for (var i = 0; i < item.pImage.length; i++) {
						...
						var imgTitle = 'Изображение ' + i;
						if (i == 0) imgTitle = 'Основное изображение';
				...
				text += '<a href="' + env.getImageDownloadLink(item.pImage, true) + '" target="_blank">Основное изображение</a>' + '<br>';
			}
			
			return text;
		}
		
		addTipToEl(el, getMessage, false, 300);	
	}
	...
	function initWorktop() {
        ...
        var tweakButton = createMainMenuButton('Настройки', function() { 
    ...
    this.showTweaksDialog = function(tabActive) {
		...
		output += '<table>';
		output += '<tr><td colspan="2"><h3>Настройки сетки элементов</h3></td></tr>';		
		
		
        output += '<tr><td>Максимальная высота одной строки :</td> <td><input type="text" class="' + env.className + 'GridRowHeight" value="' +  fav.coptions.grid.rowHeight + '"></td></tr>';
		output += '<tr><td>Допустимая погрешность высоты строки :</td> <td><input type="text" class="' + env.className + 'GridHeightDiff" value="' +  fav.coptions.grid.heightDiff + '"></td></tr>';
		output += '<tr><td>Минимальное кол-во элементов в строке :</td> <td><input type="text" class="' + env.className + 'GridMin" value="' +  fav.coptions.grid.min + '"></td></tr>';
        output += '<tr><td>Фиксированное кол-во элементов на строку :</td> <td><input type="text" class="' + env.className + 'GridFixed" value="' +  (!fav.coptions.grid.fixed ? '' : fav.coptions.grid.fixed) + '"></td></tr>';
        output += '<tr><td>Стиль по умолчанию для элемента строки :</td> <td><input type="text" class="' + env.className + 'GridCssItem" value="' +  fav.coptions.grid.cssItem + '"></td></tr>';
        output += '<tr><td>Элементов на страницу :</td> <td><input type="text" class="' + env.className + 'GridPerPage" value="' +  fav.coptions.grid.perPage + '"></td></tr>';
        ...
        */
		...
		output += '<table>';
		output += '<tr><td>Игнорировать комментарии :</td> <td><input type="text" class="kellyBlockcomments" value="' + varListToStr(fav.coptions.comments_blacklist) + '"></td></tr>';
        output += '<tr><td>Игнорировать посты :</td> <td><input type="text" class="kellyBlockposts" value="' +  varListToStr(fav.coptions.posts_blacklist) + '"></td></tr>';
        ...
        for (var i = 0; i <= displayedItems.length-1; i++) {
			...
			itemBlock.setAttribute('itemIndex', index);
                ...
                if (!previewImage) {
					...
					if (!catAnimateGif || !item.pw) previewImage = getStaticImage(previewImage);
    ...
    this.showRemoveFromFavDialog = function(itemIndex, postBlock, onRemove, onCancel, onApply) {
        ...
        var html = '<p>Подтвердите удаление</p>';
            html += '<p><a href="#" class="' + env.className + 'Remove">Удалить</a><a href="#" class="' + env.className + 'Apply">Применить изменения</a>';
            html += '<a href="#" class="' + env.className + 'Cancel">Отменить</a></p>';       
    ...
    this.categoryCreate = function() {
        ...
        var name = KellyTools.getElementByClass(modalBoxContent, env.className + 'CatName').value;
        name = name.trim();
                
        var catIsNSFW = KellyTools.getElementByClass(modalBoxContent, env.className + 'CatIsNSFW');
    ...
    this.onDownloadNativeFavPage = function(worker, thread, jobsLength) {
        ...
        KellyTools.getElementByClass(document, env.className + '-download-process').innerHTML = 'Страниц в очереди ' + jobsLength;
    ...
    this.showNativeFavoritePageInfo = function() {
        ...
        if (favPageInfo && favPageInfo.items) {
			...
			<div class="' + env.className + '-download-container">\
				 <h2>Сделать локальную копию</h2>\
				 Страниц : ' + favPageInfo.pages + ' (' + items + ')<br>\
				 Укажите список страниц выборки, которые необходимо скачать. Например 2, 4, 66-99, 44, 78, 8-9, 29-77 и т.п.,<br>\
				 Если нужно захватить все страницы оставьте не заполненным<br>\
				 <input class="kelly-PageArray" type="text" placeholder="Страницы" value=""><br>\
				 <label><input type="checkbox" class="' + env.className + '-skip-empty"> Пропускать публикации не имеющие медиа данных (текст, заблокировано цензурой)</label>\
				 <br><a href="#" class="kelly-DownloadFav">Запустить импорт</a>\
				 <a href="#" class="' + env.className + '-download-show-log" style="display : none;">Показать лог</a>\
				 ' + saveBlock + '\

var KellyProfileJoyreactorUnlock = {
    ...
    postMaxHeight : 2000, cacheLimit : 400, cacheCleanUpN : 100, ratingUnhideAfterHours : 48, ratingMaxVoteHours : 48, commentMaxDeleteMinutes : 10, // unhide rating for comments older > 24 hour
    tplItems : ['att-image', 'att-youtube', 'att-coub', 'query', 'query-post', 'query-post-with-comments', 'post', 'post-locked', 'comment', 'post-form-comment', 'post-form-vote', 'comment-form-vote'],
    ...
    formatCensoredPost : function(postBlock) {
        ...
        postId = postId[0], cImage = postBlock.getElementsByTagName('img'), cImageItem = cImage[cImage.length-1];
        if (cImage.length == 0 || (cImageItem.src.indexOf('/images/censorship') == -1 && cImageItem.src.indexOf('/images/unsafe_ru') == -1)) return false;
        
        var postContainer = KellyTools.setHTMLData(document.createElement('DIV'), this.getTpl('post-locked', {
            CLASSNAME : uClassName, 
            POST_ID : postId, 
            MODE : self.options.unlock.censoredMode, 
            NOTICE : self.options.unlock.censoredMode == 'auto' ? 'Загружаю заблокированные данные...' : 'Кликни на картинку или "Комментарии" для восстановления...',
        }));            
        
        cImageItem.parentNode.insertBefore(postContainer, cImage[cImage.length-1]);
        cImageItem.parentElement.removeChild(cImageItem);
        KellyTools.getElementByClass(postContainer, uClassName + '-censored').onclick = unlockManualAction;
    ...
    formatCensoredList : function(list) {
        this.initWorkspace(function() {
            for (var i = 0; i < list.length; i++) KellyProfileJoyreactorUnlock.formatCensoredPost(list[i]);
        });
    },
            ...
            if (self.options.unlock.auth) {
                KellyTools.injectAddition('dispetcher', function() {
                    ...
                    KellyTools.addEventPListener(document, "click", function(e) {
                        ...
                        if (self.ready && e && e.target && e.target.tagName == 'INPUT' && e.target.classList.contains('post_comment_form_unlocked') && !self.authData.postRequest && !self.unlockPool.request && self.authData.token ) {
    ...
    init : function(env) {        
        this.handler = env, this.options = env.fav.getGlobal('fav').coptions;
        ...
        if (censoredData.length > 0) this.formatCensoredList(censoredData);
    },

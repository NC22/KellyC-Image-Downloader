var KellyProfileJoyreactorUnlock = {
    ...
    onPoolUnlockedDataReady : function(rids, unlockedData, pool) {
        ...
        for (var i = 0; i < rids.length; i++) {
            ...
            if (poolItem.mediaBlock) {
                KellyTools.setHTMLData(poolItem.mediaBlock, self.getTpl('post', {PICS : self.getPublicationAttributesHtml(postUnlockedData.attributes)})); 
    ...
    formatCensoredPost : function(postBlock) {
        ...
        var unlockData = {
            ...
            mediaBlock : KellyTools.setHTMLData(document.createElement('DIV'), this.getTpl('post-locked', {
                CLASSNAME : uClassName, 
                POST_ID : postId, 
                MODE : self.options.unlock.censoredMode, 
                NOTICE : self.options.unlock.censoredMode == 'auto' && !self.unlockPool.request ? 'Загружаю заблокированные данные...' : 'Кликни на картинку или "Комментарии" для восстановления...',
            })), 

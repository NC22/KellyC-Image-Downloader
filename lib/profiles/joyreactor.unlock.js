var KellyProfileJoyreactorUnlock = {
    ...
    updatePostBounds : function(postData) {
          var cImage = postData ? postData.getElementsByTagName('img') : [];
          if (cImage.length > 0) {
              cImage[0].onload = function() {
                if (this.getBoundingClientRect().height < KellyProfileJoyreactorUnlock.postMaxHeight) {
    ...
    unlockPostList : function(ids, callback, tpl) {
        ...
        var unlockController = {};
            ...
            unlockController.maxAttempts = 6;
        ...
        self.unlockPostList(ids, function(html, rids) {
            ...
            for (var postId in lastPool) {
                if (html[postId]) {
                    ...
                    self.updatePostBounds(lastPool[postId].postData);
    ...
    unlockPost : function(postId, postBlock, postContainer, commentsContainer, callback) {
        ...
        self.unlockPostList([postId], function(html, ids, responseData) {
            ...
            self.updatePostBounds(postContainer);
    ...
    formatIfCensored : function(handler, postBlock, autoUnlock) {
        ...
        for (var b = 0; b < cImage.length; b++) {
            ...
            if (autoUnlock) {
                self.unlockPool[postId] = {postId : postId, post : postBlock, postData : postContainer, notice : KellyTools.getElementByClass(postContainer, handler.className + '-censored-notice')};

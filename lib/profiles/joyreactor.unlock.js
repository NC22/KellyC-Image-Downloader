var KellyProfileJoyreactorUnlock = {
    ...
    postMaxHeight : 2000, cacheLimit : 1000, cacheCleanUpN : 900, ratingUnhideAfterHours : 24, ratingMaxVoteHours : 48, // unhide rating for comments older > 24 hour
    ...
    getTpl(tplName, data) {
        var html = this.tplData[tplName].data;
        if (data) for (var k in data) html = KellyTools.replaceAll(html, '__' + k + '__', data[k]);
        return html;
    },
    
    getVoteForm(form, id, publicationDate, rating, hideSymbol) {
        
        if (!this.authData || !this.authData.time || this.authData.time - publicationDate > this.ratingMaxVoteHours * 60 * 60 * 1000 ) form += '-readonly';
        
        if (form.indexOf('readonly') == -1 && this.authData && this.authData.time && this.authData.time - publicationDate < this.ratingUnhideAfterHours * 60 * 60 * 1000) rating = hideSymbol;
        else rating = KellyTools.val(rating, 'float').toFixed(1);
        
        return this.getTpl(form, {CONTENT_ID : id, RATING : rating});
    },
    ...
    onPoolUnlockedDataReady : function(rids, unlockedData, pool) {
        ...
        for (var i = 0; i < rids.length; i++) {
            ...
            var htmlPostCommentForm = (self.authData ? self.getTpl('post-form-comment', { POST_ID : postId, AUTH_TOKEN : self.authData.token}) : '');
                ...
                if (postUnlockedData.comments && postUnlockedData.comments.length > 0) {
                    postUnlockedData.comments.forEach(function(comment) { 
                        ...
                        htmlComments += self.getTpl('comment', { 
                            ...
                            VOTE : self.getVoteForm('comment-form-vote', self.getNodeId(comment.id), datetime, comment.rating, '≈0'),
                        });
            ...
            if (self.authData && poolItem.ratingBlock) {
    ...
    formatIfCensored : function(postBlock) {
        ...
        for (var b = 0; b < cImage.length; b++) {
            ...
            var unlockInitiator = false, unlockDataOnReady = function(success, errorText) {
                
                if (errorText) KellyTools.getElementByClass(postContainer, uClassName + '-censored-notice').innerText = errorText;                
                if (unlockInitiator) self.showCNotice(success ? false : errorText);
                
                if (success && unlockInitiator) {
                    self.handler.formatPostContainer(unlockData.postBlock);
                    if (unlockData.commentsBlock && unlockInitiator.className.indexOf('comment') != -1) unlockData.commentsBlock.style.display = '';                
                    if (unlockData.commentsBlock && unlockInitiator.className.indexOf('comment') != -1) setTimeout(function() { window.scrollTo(0, unlockData.commentsBlock.getBoundingClientRect().top + KellyTools.getScrollTop() - 90); }, 200);       
                }
                return false;
            };
            
            var unlockData = {postId : postId, postBlock : postBlock, ratingBlock : KellyTools.getElementByClass(postBlock, 'post_rating'), commentsBlock : KellyTools.getElementByClass(postBlock, 'post_comment_list'), postData : postContainer, onReady : unlockDataOnReady};
                ...
                KellyTools.injectAddition('dispetcher', function() {     
                    KellyTools.addEventPListener(window, "message", function(e) {
                        if (e && e.data && e.data.method == 'kelly_dynaminc.getvar' && e.data.senderId == 'dynamic_dispetcher') {
                            self.authData = {time : new Date(parseInt(e.data.varList.server_time) * 1000), token : e.data.varList.token, user_id : e.data.varList.user_id};
                            self.formatPosts();

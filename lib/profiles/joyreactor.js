...
function KellyProfileJoyreactor() {
    ...
    this.setLocation = function(location) {
        ...
        handler.webRequestsRules = {
            ...
            urlMap : [['https://api.joyreactor.cc/graphql', 'top.reactor.cc']],
    ...
    this.checkCensorship = function(postBlock) {
        
        var postId = postBlock.id.match(/[0-9]+/g);
        if (postId.length <= 0) return;
        
        var showCNotice = function(text) {
            var tooltip = handler.fav.getTooltip();
        ...
        for (var b = 0; b < cImage.length; b++) {
            ...
            cImage[b].parentNode.insertBefore(
                KellyTools.setHTMLData(document.createElement('DIV'), '<div class="' + handler.className + '-censored-notice">Заблокированный пост. Кликни на картинку для восстановления</div>'), 
                cImage[b]
            );
            
            cImage[b].classList.add(handler.className + '-censored');
            cImage[b].setAttribute('data-censored-src', cImage[b].src);
            cImage[b].onclick = function() {                       
                   
                var self = this;
                if (self.getAttribute('data-inuse')) return showCNotice('Дождитесь окончания загрузки...');
                
                showCNotice('Загрузка...');                
                self.setAttribute('data-inuse', '1');
                
                KellyTools.fetchRequest('https://api.joyreactor.cc/graphql', {
                    method : 'POST', 
                    headers : {'Content-Type': 'application/json'},
                    body: "{\"operationName\":null,\"variables\":{},\"query\":\"{\\n  node(id: \\\"" + window.btoa('Post:' + postId) + "\\\") {... on Post {attributes {id, image {type}}}}}\"}",
                    responseType : 'json',
                }, function(url, responseData, responseStatusCode, error, fetchRequest) {
                    
                    self.removeAttribute('data-inuse');
                    if (error || !responseData || !responseData.data.node) return showCNotice('Ошибка обработки данных');
                        
                    var html = '';
                    responseData.data.node.attributes.forEach(function(imageData) {
                        
                        var src = window.atob(imageData.id).split(':')[1];
                            src = "//img10.joyreactor.cc/pics/post/" + 'post-' + src + '.' + (imageData.image && imageData.image.type ? imageData.image.type.toLowerCase() : 'jpg');
                        
                        html += '<div class="image"><a href="' + src.replace('post/', 'post/full/') + '" class="prettyPhotoLink" target="_blank" rel="prettyPhoto"><img src="' + src  + '"></a></div>';
                    });
                                            
                    var cPostContent = KellyTools.setHTMLData(document.createElement('DIV'), html);
                        cPostContent.className = 'post_content';
                        cPostContent.style.maxHeight = 'unset';
                    
                    self.parentElement.insertBefore(cPostContent, self);
                    self.parentElement.removeChild(self);
                    
                    KellyTools.getElementByClass(postBlock, handler.className + '-censored-notice').parentElement.style.display = 'none';                        
                    handler.formatPostContainer(postBlock);
                    showCNotice(false);
                });
            };
    ...
    this.formatPostContainer = function(postBlock) {
        ...
        if (coptions.unlockCensored) handler.checkCensorship(postBlock);

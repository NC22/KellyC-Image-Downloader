...
function KellyOptions(cfg) {
    ...
    this.favEnv = false; this.wrap = false;    
    this.tabData = {'BaseOptions' : {loc : 'options_main'}, 'Storage' : {loc : 'storage'}, 'Cfg' : {loc : 'restore'}, 'Other' : {loc : 'other'},};
    ...
    this.cfgInput = {     
        ...
        'hideAddToFav': {loc : 'hide_add_to_fav', type : 'bool'},
        ...
        'mobileOptimization': {loc : 'mobile_optimization', type : 'bool'},
    ...
    this.showSection = function(key, close) {
        ...
        var output = '<h3 data-section="' + env.className + '-options-' + key + '">' + lng.s('', key) + '</h3>';
    ...
    this.showCfgItem = function(cfgKey) { 
        ...
        if (handler.cfgInput[cfgKey].noticeUp) output += '<tr><td colspan="2">' + lng.s('', 'grabber_options_notice') + '</td></tr>';
        ...
        } else if (type == 'folder' || type == 'string' || type == 'int') {
    ...
    this.showCfgPool = function(input) {
        ...
        if (typeof input == 'string') {
            for (var cfgKey in handler.cfgInput) if (cfgKey.indexOf(input) != -1) list.push(cfgKey);
        } else list = input;

        for (var i = 0; i < list.length; i++) output += handler.showCfgItem(list[i]);
        return output;
    }
    
    this.showSave = function() {        
        return '<div><input type="submit" value="' + lng.s('Сохранить', 'save') + '" class="' + env.className + '-OptionsSave"></div>';
    }
    ...
    this.applyInputSettingsPool = function() {
        ...
        for (var cfgKey in handler.cfgInput) {
            ...
            if (handler.cfgInput[cfgKey].type == 'bool') {
            ...
            } else if (handler.cfgInput[cfgKey].type == 'enum') {
            ...
            } else if (handler.cfgInput[cfgKey].type == 'select') {
                console.log(input.value); console.log(handler.cfgInput[cfgKey].list.indexOf(input.value)); 
                if (handler.cfgInput[cfgKey].list.indexOf(input.value) != -1) handler.setCfgValue(cfgKey, input.value);
                
            }  else if (handler.cfgInput[cfgKey].type == 'string' || handler.cfgInput[cfgKey].type == 'int') {
                
                 handler.setCfgValue(cfgKey, KellyTools.val(input.value, handler.cfgInput[cfgKey].type));  
                 
            } else if (handler.cfgInput[cfgKey].type == 'folder') {
    ...
    this.updateEvents = function() {
        ...
        var tabActivate = function(tab) {        
            ...
            KellyTools.getElementByClass(handler.wrap, env.className + '-OptionsMessage').innerHTML = '';                
            KellyTools.getElementByClass(handler.wrap, handler.tabActive).style.display = 'block';
            KellyTools.classList('add', tab, 'active');
            
            if (handler.tabActive == env.className + '-Storage') {
            ...
            } else if (handler.tabActive == env.className + '-Cfg') {
    ...
    this.showOptionsDialog = function(selectTab) {
        ...
        for (var tabKey in handler.tabData) {       
            ...
            if (tabKey == 'BaseOptions') {
                output += handler.showSection('options_fav_add') + handler.showCfgPool(['syncByAdd', 'hideAddToFav', 'hideSoc', 'addToFavNoConfirm', 'addToFavSide']) + handler.showSection('options_fav_add', true);        
                output += handler.showSection('fast_download') + handler.showCfgPool('fastsave_') + handler.showSection('fast_download', true);             
                output += handler.showSection('cgrid_tiles_header') + handler.showCfgPool(['newFirst']) + handler.showCfgPool('grid_') + handler.showSection('cgrid_tiles_header', true);
                output += handler.showSection('grabber_options_title') + handler.showCfgPool('grabberDriver_') + handler.showSection('grabber_options_title', true);       
                output += handler.showSave();
                output = output.replace('[ICON]', htmlFastIcon).replace('[ICON2]', htmlFastIcon.replace('icon-download', 'icon-download-configurable'));        
            } else if (tabKey == 'Other') {
                output += '<table>' + handler.showCfgPool(['mobileOptimization', 'darkTheme', 'optionsSide', 'hideTopMenu', 'debug', 'webRequest', 'baseFolder']);
                if (env.hostClass != 'options_page') output += '<tr><td colspan="2"><label>' + KellyTools.getProgName(env.location) + '</label></td></tr>';                  
                output += '</table>' + handler.showSave();   
            }
            output += '</div>';
    ...
    this.updateOptionsConfig = function() {
        ...
        KellyTools.getElementByClass(handler.wrap, env.className + '-OptionsMessage').innerText = lng.s('Настройки сохранены', 'options_saved');

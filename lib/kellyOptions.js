...
function KellyOptions(cfg) {
    ...
    this.showOptionsDialog = function(tabActive) {
        ...
        if (handler.favEnv.isDownloadSupported) {
            ...
            output += '<table class="' + env.className + '-options-table">\
                ...
                <tr><td colspan="2">\
                    <label><input type="checkbox" value="1" class="' + env.className + 'FastSaveCheck" ' + (fav.coptions.fastsave.check ? 'checked' : '') + '> ' + lng.s('Проверять был ли уже скачан файл', 'fast_save_check') + ' (beta)</label>\
        ...
        if (handler.favEnv.isDownloadSupported) {
            ...
            // if (fav.coptions.debug) output += '<option value="' + KellyGrabber.REQUEST_FETCHBG + '" ' + (fav.coptions.grabberDriver.requestMethod == KellyGrabber.REQUEST_FETCHBG ? 'selected' : '') + '>Fetch BG [experimental]</option>';
        ...
        // todo 
        // output += '<tr><td colspan="2"><label><input type="checkbox" value="1" class="' + env.className + 'OptionsShowVideo" ' + (fav.coptions.showVideo ? 'checked' : '') + '> \
        ...
        output += '<tr><td colspan="2"><label><input type="checkbox" class="' + env.className + 'OptionsWebRequest" ' + (fav.coptions.webRequest ? 'checked' : '') + '> WebRequest API</label></td></tr>';           
        ...
        if (env.hostClass != 'options_page') output += '<tr><td colspan="2"><label>' + KellyTools.getProgName(env.location) + '</label></td></tr>';
    ...
    this.updateOptionsConfig = function() {
        ...
        if (KellyTools.getElementByClass(favContent, env.className + 'OptionsWebRequest').checked) {

function KellyThreadWork(cfg) {
    ...
    var jobs = [];
    var env = false;
    var maxThreads = 2; // лучше не использовать больше - бан на несколько часов
    var threads = [];
    var iframeC = 0;
	
    var timeout = 15; // таймаут ожидания загрузки страницы в секундах, в обработчик onLoad попадет idocument = false
	var timeoutOnEnd = [2,1,3,4]; // таймер перехода к следующей задаче после выполнения - сек. рандомно из массива
        ...
        for (var i = 0; i < threads.length; i++) {
            if (threads[i].frame == thread.frame) {     
				...
				if (!jobs.length && !threads.length) {   
					...
					setTimeout(function() {        
						 applayJob();
					}, timeoutOnEnd[Math.floor(Math.random() * ((timeoutOnEnd.length - 1) + 1))] * 1000);
				}

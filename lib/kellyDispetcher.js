...
var KellyEDispetcher = new Object;
    ...
    KellyEDispetcher.onDownloaderConnect = function(port) {
        ...
        var tabData = {port : port, tab : port.sender.tab, id : port.sender.tab.id, disable : true};
        ...
        var onDownloaderMessage = function(request) {
            ...
            if (request.method == 'registerDownloader') {
                resetEvents(); 
                tabData.disable = request.disable ? true : false;
                
                if (!tabData.disable) {
                    tabData.cors = request.cors;
                    tabData.referrer = request.referrer;
                    tabData.types = typeof request.types != 'undefined' ? request.types : ['xmlhttprequest'];
        ...
        port.onDisconnect.addListener(function(p){
            ...
            KellyTools.log('DISCONECTED | Reason : ' + (p.error ? p.error : 'unknown'), 'KellyEDispetcher [PORT]');
            resetEvents();
            
            for (var i = 0; i < KellyEDispetcher.downloaderTabs.length; i++) {
                if (KellyEDispetcher.downloaderTabs[i].id == port.sender.tab.id) {
                    KellyEDispetcher.downloaderTabs.splice(i, 1);      
                    break;
                }
            }
        });

...
var KellyEDispetcher = new Object;
    ...
    KellyEDispetcher.addRequestListeners = function(tabData) {
        ...
        if (KellyTools.getBrowserName() != 'firefox') { // webRequests frome extension returns tabId = -1 for firefox for content-script events
        ...
        if (tabData.cors) {    
            ...
            tabData.onHeadersReceived = function(e) {  
                   ...
                   if (tabData.urlMap && getHeader(e.responseHeaders, 'status') == '301') { // extend url map list with redirect links for use in (onBeforeRequest | onHeadersReceived) on new request
                       var referrer = getRulesDataForUrl(e.url);
                   ...
                   if (tabData.referrer && e.type == "xmlhttprequest" && getHeader(e.responseHeaders, 'status') == '301' && tabData.referrer.indexOf('https') != -1) {
            ...
            return true; // async mode
                ...
                for (var i = 0; i < request.items.length; i++) {
                    ...
                    if (!loadedCss[request.items[i]].url) {
                        ...
                        KellyTools.readUrl(loadedCss[request.items[i]].url, onSuccess, onFail, 'GET', true, "text/css"); 
                    }
                }

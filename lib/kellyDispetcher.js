...
var KellyEventsDispetcher = new Object;
    ...
    KellyEventsDispetcher.isDownloadSupported = function() {
    
        if (!KellyEventsDispetcher.api || typeof KellyEventsDispetcher.api.downloads == 'undefined') {
            return false;
        }
	...
	KellyEventsDispetcher.onMessage = function(request, sender, callback) {
		...
		if (request.method == 'downloads.cancel') {        
			...
			response.downloadId = false;
			response.operationId = false;
            ...
            response.isDownloadSupported =  KellyEventsDispetcher.isDownloadSupported();
			...
			KellyTools.getBrowser().downloads.onChanged.addListener(
				function(downloadDelta) {
					...
					for (var i=0; i <= KellyEventsDispetcher.tabEvents['onChanged'].length-1; ++i) {
						KellyTools.getBrowser().tabs.sendMessage(KellyEventsDispetcher.tabEvents['onChanged'][i], {method: "onChanged", downloadDelta : downloadDelta}, function(response) {});
					}
				}
			);
        ...
        // loads selected css items and send an init dispetcher env data
                ...
                if (loaded == request.items.length) {
                    ...
                    if (callback) {
                        ...
                        callback({
                            ...
                            data : answerData,
                            isDownloadSupported : KellyEventsDispetcher.isDownloadSupported(),
                        });
                    }
                }
            }
            ...
            for (var i = 0; i < request.items.length; i++) {
               console.log(request.items[i]);
                var css = KellyTools.getBrowser().runtime.getURL(KellyEventsDispetcher.envDir + 'css/' + request.items[i] + '.css');

    ...
    */
    ...
    this.load = function(type, onAfterload, context) {
        ...
        var onLoadItems = function(itemsDb) {
            ...
            for (var k in itemsDb) fav[k] = itemsDb[k];            
            page = 1;
    ...
    function initWorktop() {
        ...
        var iconHtml = '<div class="' + env.className + '-icon ' + env.className + '-icon-diskete ' + env.className + '-buttoncolor-dynamic"></div>';        
    ...
    this.getOptionsManager = function() {
        ...
        if (!optionsManager) optionsManager = new KellyOptions({favEnv : handler, wrap : favContent});       
    ...
    function updateDisplayItemsList() {
        ...
        for (var i = fav.coptions.newFirst ? fav.items.length-1 : 0; fav.coptions.newFirst ? i >= 0 : i < fav.items.length; fav.coptions.newFirst ? i-- : i++) {
            ...
            if (aFilters && aFilters.srcUrl) {
                if (!fav.items[i].pImage) continue;
                
                if (aFilters.reg) {
                    var reg = new RegExp(aFilters.srcUrl), match = typeof fav.items[i].pImage == 'string' ? fav.items[i].pImage.match(reg) : fav.items[i].pImage[0].match(reg);                    
                } else {
                    var match = typeof fav.items[i].pImage == 'string' ? fav.items[i].pImage.indexOf(aFilters.srcUrl) != -1 : fav.items[i].pImage[0].indexOf(aFilters.srcUrl) != -1 ;        
                }
    ...
    function updateGoToPageButton(gotoPage) {
        ...
        KellyTools.classList(handler.getFavPageListCount() > 14 ? 'add' : 'remove', gotoPage, env.className + '-active');
    }
    ...
    this.updateFilteredData = function() {
        ...
        var tiles = handler.getImageGrid().getTiles();   
        if (tiles && tiles.length) window.scrollTo(0, tiles[0].getBoundingClientRect().top + KellyTools.getScrollTop() - 90);
    }
    ...
    function showItem(item, subItem) {
        ...
        // show as image
            ...
            if (item.vd) {
                var html = '\
                <video class="' + env.className + '-preview ' + env.className + '-media" data-width="' + (!item.pw ? '300' : item.pw) + '" data-height="' + (!item.ph ? '300' : item.ph) + '" loop="" muted="">\
    ...
    function showGoToPageTooltip(target) {
        ...
        KellyTools.getElementByClass(container, env.className + '-Go').onclick = function() { 
            ...
            handler.goToFavPage(KellyTools.inputVal(env.className + '-GoToPage', 'int', container));
    ...
    function checkDataFilterLock() {
        ...
        if (handler.dataFilterLock || handler.getDownloadManager().getState() != 'wait') {
            
            var tooltip = KellyTools.getNoticeTooltip(env.hostClass, env.className);
                tooltip.getContent().innerText = handler.dataFilterLock ? handler.dataFilterLock.message : 'Перед выполнением действия необходимо остановить загрузку данных';
                tooltip.show(true);
        ...
        if (!favNativeParser || (favNativeParser && !favNativeParser.isBeasy())) {  
            ...
            var showDownloadManagerForm = function(show) {
                ...
                if (!show) {
                    ...
                    fav.coptions.grabber.titleUpdate = false;
        ...
        if (enabled) {
            ...
            for (var i = 0; i < sectionItems.length; i++) {
                ...
                var el = document.createElement('div');
                    ...
                    el.innerText = lng.s('', sectionItems[i].getAttribute('data-title'));
    ...
    this.closeSidebar = function(action) {        
        ...
        if (action == 'fav' && KellyTools.classList('contains', document.body, env.className + '-mobile')) showInlineSidebar(false);               
    ...
    this.showSidebar = function(hideHeader, onClose, action) {
        ...
        if (action == 'fav' && KellyTools.classList('contains', document.body, env.className + '-mobile')) showInlineSidebar(true);
    ...
    this.showAddToFavDialog = function(postBlock, comment, onAdd, onRemove, onClose) {
        ...
        handler.getStorageManager().showUnsafeStoreWarning('add_to_fav');
        handler.showSidebarMessage(false);
    ...
    this.categoryCreate = function(container) {
        ...
        var itemIndex = handler.getStorageManager().categoryCreate(fav, name, catIsNSFW, orderNum);
        if (itemIndex === false) {
    ...
    this.initResources = function(resources) {
        ...
        KellyTools.getBrowser().runtime.sendMessage({method: "getResources", items : resources ? resources : ['main', env.profile]}, function(request) {
             if (!request || !request.data.css) {                
                log('onLoadCssResource : css empty');
                log('onLoadCssResource : Browser API response : ' + request.data.notice);                
                return false; 
            }
            
            KellyTools.addCss(env.className + '-mainCss', KellyTools.replaceAll(request.data.css, '__BASECLASS__', env.className));                        
            initWorktop();            

    ...
    */
    ...
    this.onDownloadNativeFavPage = function(worker, thread, jobsLength) {
        ...
        for (var i = posts.length - 1; i >= 0; i--) {         
            ...
            if (fav.coptions.unlock.censoredParser && !selectedImages.length && KellyProfileJoyreactorUnlock.formatIfCensored(env, selectedPost) === true) {
        ...
        if (fav.coptions.unlock.censoredParser && pageInfo.censoredPostIds.length) {
            ...
            var unlockRequest = KellyProfileJoyreactorUnlock.unlockPostList(pageInfo.censoredPostIds, function(htmlByPostId, postIds) {
                ...
                for (var i = 0; i < pageInfo.censoredPostIds.length; i++) {
                    ...
                    if (htmlByPostId === false || !htmlByPostId[blockPostId]) {
                    ...
                    favNativeParser.addToLog('Картинки для заблокированного поста ' + blockPostId + ' найдены - ' + (typeof item.pImage == 'string' ? 1 : item.pImage.length));
                }
    ...
    this.getBookmarksParser = function() {
        ...
        if (typeof KellyProfileJoyreactorUnlock != 'undefined') {
            ...
            fav.coptions.unlock.censoredParser = false;
        }
    ...
    this.showNativeFavoritePageInfo = function() {
        ...
        // для текстовых публикаций делать заголовок через метод setSelectionInfo
        ...
        var html = '\
            ...
            <div class="' + env.className + '-exporter-container hidden">\
                 ' +  lng.s('Страниц', 'pages_n') + ' : <input class="' + env.className +'-PagesN" type="text" placeholder="' + lng.s('Страниц', 'pages_n')+ '" value="' + favPageInfo.pages + '"><br>\

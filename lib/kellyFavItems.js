    ...
    */
    ...
    this.load = function(type, onAfterload) {
        ...
        var onLoadItems = function(itemsDb) {
            ...
            if ((type == 'items' || !type) && onAfterload) onAfterload(fav); 
        }
    ...
    this.showItemInfo = function(item) {    
        ...
        if (item.name) text += lng.s('Подпись : ', 'item_sign') + '<br>' + item.name + '<br>';
        ...
        if (item.link) text += '<a href="' + KellyTools.validateUrlForLocation(item.link, env.location) + '" target="_blank">' + lng.s('Показать пост', 'go_to_publication') + '</a>' + '<br>';        
    ...
    this.showFavouriteImages = function() {
        ...
        if (!env.isNSFW() || fav.coptions.ignoreNSFW) handler.ignoreNSFW();
    ...
    this.initWebRequestRules = function(data) {
    
        if (fav.coptions && !fav.coptions.webRequest) {
            log('webRequests API blocking is Disabled by config');
            return false;
        }
                
        if (!data) data = env.webRequestsRules;  
    ...
    this.initResources = function(resources) {
        ...
        KellyTools.getBrowser().runtime.sendMessage({method: "getResources", items : resources ? resources : ['main', env.profile]}, function(request) {
            ...
            handler.addCss(KellyTools.replaceAll(request.data.css, '__BASECLASS__', env.className));             
            ...
            } else handler.initWebRequestRules(false);

    ...
    */
    ...
    this.onDownloadNativeFavPage = function(worker, thread, jobsLength) {
        ...
        for (var i = posts.length - 1; i >= 0; i--) {         
            ...
            if (!selectedImages.length && favNativeParser.unlockRequests !== false && KellyProfileJoyreactorUnlock.formatIfCensored(env, selectedPost) === true) {
        ...
        if (favNativeParser.unlockRequests !== false && pageInfo.censoredPostIds.length) {
            
            favNativeParser.addToLog('Восстанавливаю заблокированный контент...');
            favNativeParser.pause(true);
            
            var unlockRequest = KellyProfileJoyreactorUnlock.unlockPostList(pageInfo.censoredPostIds, function(htmlByPostId, postIds) {
                
                for (var i = 0; i < pageInfo.censoredPostIds.length; i++) {
                    var blockPostId = pageInfo.censoredPostIds[i];
                    if (!htmlByPostId[blockPostId]) {
                        
                        // todo mark as bad
                        log('Данные для заблокированного поста отсутствуют ' + pageInfo.censoredPostIds[i]);
                        continue;
                    }
                    
                    var itemIndex = pageInfo.censoredPosts[blockPostId].itemIndex;
                    KellyTools.setHTMLData(pageInfo.censoredPosts[blockPostId].post, KellyProfileJoyreactorUnlock.getTpl('post', {PICS : htmlByPostId[blockPostId]}));    
                    worker.collectedData.items[itemIndex].pImage = env.getAllMedia(pageInfo.censoredPosts[blockPostId].post);
                    
                    favNativeParser.addToLog('Картинки для заблокированного поста ' + blockPostId + ' найдены - ' + worker.collectedData.items[itemIndex].pImage.length);
                }
    ...
    this.downloadNativeFavPage = function(el) {
        ...
        if (favNativeParser.getJobs().length) {
            ...
            favNativeParser.stopUnlockRequests();
    ...
    this.getBookmarksParser = function() {
        ...
        if (typeof KellyProfileJoyreactorUnlock != 'undefined') {
    ...
    this.showNativeFavoritePageInfo = function() {
        ...
        if (typeof KellyProfileJoyreactorUnlock != 'undefined') {
            unlockPostsHtml = '\
            <label><input type="checkbox" class="' + env.className + '-exporter-skip-empty" ' + (fav.coptions.unlock.censoredParser ? 'checked' : '') + '> ' +  
                lng.s('', 'unlock_censored_parser') +
             '</label>';
        }
    ...
    this.initResources = function(resources) {
        ...
        KellyTools.getBrowser().runtime.sendMessage({method: "getResources", items : resources ? resources : ['core', env.profile]}, function(request) {

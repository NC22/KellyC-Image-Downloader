...
function KellyFavItems(cfg) 
{
    ...
    */
    ...
    this.showAddToFavDialog = function(postBlock, comment, onAdd, onRemove, onClose) {
        ...
        var loadDimensions = selectedInfo && selectedInfo['dimensions'] ? false : true; // dimensions can be already initialized throw getAllMedia method     
        var controllsAndPreview = getSelectedPostMediaControlls(loadDimensions); // selected preview + controlls
        ...
        var catsHTML = '<option value="-1">' + lng.s('Без категории', 'cat_no_cat') + '</option><option value="-2">[' +lng.s('Создать категорию', 'cat_create') + ']</option>';
        
        for (var i = 0; i < fav.categories.length; i++) {
            catsHTML += '<option value="' + fav.categories[i].id + '">' + fav.categories[i].name + '</option>';
        }
        ...
        var html = '\
            <div class="' + env.className + 'SavePostWrap">\
                    <div class="' + env.className + 'CatAddForm" style="display : none;"><div>\
                    ...
                    <div><a href="#" class="' + env.className + 'Remove" ' + (!existItem ? 'style="display : none;"' : '') + '>' + lng.s('Удалить', 'delete') + '</a></div>\
            </div>';
    ...
    function getFavItemFromSelected(postItem) {
        ...
        if (!postItem) {
            postItem = { 
                ...
                // commentLink : '', text : '', pw : 0, ph : 0, ps : 0, -- optional parametrs
            };
        }
        ...
        if (postItem.name) { 
            ...
            // set main image selected in PreviewImage element 

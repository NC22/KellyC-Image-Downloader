...
function KellyGrabber(cfg) {
    ...
    function assignEvents() {
        ...
        KellyGrabber.getBrowser().runtime.sendMessage({method: "onChanged.addListener"}, function(response) {});
                
        KellyGrabber.getBrowser().runtime.onMessage.addListener(
    ...
    this.cancelDownloads = function() {    
        ...
        for (var i=0; i <= downloads.length-1; ++i) {
            if (!downloads[i].ready && downloads[i].downloadId && downloads[i].downloadId != -1) {
                ...
                KellyGrabber.getBrowser().runtime.sendMessage({method: "downloads.cancel", downloadId : downloads[i].downloadId}, function(response) {
    ...
    this.addFile = function(filename, url) {
        ...
        var ext = KellyGrabber.getExt(url);
        if (!ext) return false;
    ...
    this.setDownloadIdByOperationId = function(operationId, downloadId) {
        ...
        if (mode == 'wait') { // perhapse promise was sended after cancel ?
            KellyGrabber.getBrowser().runtime.sendMessage({method: "downloads.cancel", downloadId : downloadId}, function(response) {
    ...
    this.createAndDownloadFile = function(data, filename, mimetype) {
        ...
        var ext = KellyGrabber.getExt(filename);
        if (!ext) ext = 'txt';
        ...
        KellyGrabber.getBrowser().runtime.sendMessage({method: "downloads.download", blob : true, operationId : -1, download : downloadOptions}, function(response){});             
    ...
    function downloadByXMLHTTPRequest(download) {
        ...
        var onSuccessBlob = function(requestObj, blobData) {
            ...
            downloadOptions.url = URL.createObjectURL(blobData);               
            KellyGrabber.getBrowser().runtime.sendMessage({method: "downloads.download", blob : true, operationId : operationId, download : downloadOptions}, function(response){});             
        }
        ...
        var onFailBlob = function(requestObj) {
            ...
            KellyGrabber.getBrowser().runtime.sendMessage({method: "downloads.download", blob : false, operationId : operationId, download : downloadOptions}, function(response){});   
    ...
    this.addDownloadWork = function() {
        ...
        for (var i=0; i <= downloads.length-1; ++i) {
            ...
            /*
            KellyGrabber.getBrowser().runtime.sendMessage({method: "downloads.download", download : {
...
KellyGrabber.getBrowser = function() {    
   return KellyTools.getBrowser();
}  

KellyGrabber.getExt = function(str, limit) {

...
KellyRecorderFilterArtstation.parseImagesDocByDriver = function(handler, data) {
    
    if (handler.url.indexOf('artstation') != -1) {
        
        var meta = data.thread.response.match(/<\meta name="twitter:image"([\s\S]*?)>/g); // todo - parse graphql data to detect multiple images for one publication
        if (meta) {                
